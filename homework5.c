...
char error_req[4096];
char error1[4096]="404 Not Found";
char error2[4096]="400 Bad Request";
...
void get_directory_content(int client_fd,char* directory_path,char* requested_file)
{
	...
	if(path != NULL)
	{
		...
		while((underlying_file = readdir(path)) != NULL)
		{
			...
			if(!S_ISDIR(checker.st_mode))
				{
					sprintf(error_req, "HTTP/1.0 %s\r\n" "Content-type: text/html; charset=UTF-8\r\n\r\n", error2);
					send(client_fd,error_req,strlen(error_req),0);
				}
			else{printf("\n");
